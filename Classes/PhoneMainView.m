...
- (void)startUp {
    ...
    @try {
        ...
        if( linphone_core_get_global_state(core) != LinphoneGlobalOn ){
            ...
            // modified by khc to disable account assistant
...
/*    int count = 0;
    ...
    NSPredicate *predicate =[NSPredicate  predicateWithFormat:@"flag_readed == %@",[NSNumber numberWithBool:NO]];
...
+ (CATransition*)getTransition:(UICompositeViewDescription *)old new:(UICompositeViewDescription *)new {
    ...
    if([old equal:[RootViewController compositeViewDescription]]) {
        ...
        if([new equal:[DialerViewController   compositeViewDescription]] ||
           ...
           [new equal:[HistoryViewController  compositeViewDescription]] ||
           [new equal:[RootViewController     compositeViewDescription]]) {
            left = true;
        }
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if(alertView.tag ==1 && buttonIndex ==0){
        NSString * numstr =  [[alertView.message componentsSeparatedByCharactersInSet:
                                                     [[NSCharacterSet decimalDigitCharacterSet] invertedSet]]
                                                    componentsJoinedByString:@""];
        ContactDetailsViewController *controller = DYNAMIC_CAST([[PhoneMainView instance] changeCurrentView:[ContactDetailsViewController compositeViewDescription] push:TRUE], ContactDetailsViewController);
        if(controller != nil) {
            
            [controller newContact:numstr];
