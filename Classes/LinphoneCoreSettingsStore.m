...
- (void)transformLinphoneCoreToKeys {
    ...
    if (cfg){
        ...
        if (addr){
            ...
            switch (linphone_address_get_transport(proxy_addr)) {
                case LinphoneTransportUdp: tname = "udp"; break;
    ...
    [self setString: lp_config_get_string(conf, LINPHONERC_APPLICATION_KEY, "xmppid_preference", NULL) forKey:@"xmppid_preference"];
    
    [self setString: lp_config_get_string(conf, LINPHONERC_APPLICATION_KEY, "xmpppsw_preference", NULL) forKey:@"xmpppsw_preference"];
    
    [self setString: lp_config_get_string(conf, LINPHONERC_APPLICATION_KEY, "xmppdomain_preference", NULL) forKey:@"xmppdomain_preference"];
 
    [self setString: lp_config_get_string(conf, LINPHONERC_APPLICATION_KEY, "voicemail_preference", NULL) forKey:@"voicemail_preference"];
...
- (void)synchronizeAccount {
    ...
    if (username && [username length] >0 && domain && [domain length]>0) {
        ...
        if( proxy_addr ){
            ...
            if      ( [transport isEqualToString:@"udp"] ) type = LinphoneTransportUdp;
...
- (BOOL)synchronize {
    ...
    NSString*  xmppid = [self stringForKey:@"xmppid_preference"];
    [[LinphoneManager instance] lpConfigSetString:xmppid forKey:@"xmppid_preference"];
    NSString*  xmpppsw = [self stringForKey:@"xmpppsw_preference"];
    [[LinphoneManager instance] lpConfigSetString:xmpppsw forKey:@"xmpppsw_preference"];
    NSString*  xmppdomain = [self stringForKey:@"xmppdomain_preference"];
    [[LinphoneManager instance] lpConfigSetString:xmppdomain forKey:@"xmppdomain_preference"];
    NSString*  voicemail = [self stringForKey:@"voicemail_preference"];
