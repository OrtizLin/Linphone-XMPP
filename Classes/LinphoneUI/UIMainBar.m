...
- (void)updateUnreadMessage:(BOOL)appear{
    ...
    NSPredicate *predicate =[NSPredicate  predicateWithFormat:@"flag_readed == %@ AND receiver.name ==%@",[NSNumber numberWithBool:NO],selfUserName];
    ...
    if (unreadMessage > 0) {
        ...
        [chatNotificationLabel setText:[NSString stringWithFormat:@"%lu", (unsigned long)unreadMessage]];
...
- (void)updateView:(UICompositeViewDescription*) view {
    ...
    if([view equal:[RootViewController compositeViewDescription]]) {
