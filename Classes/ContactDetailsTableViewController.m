    ...
    if (![self isEditing]) {
        ...
        if(contactSections[[indexPath section]] == ContactSections_Number) {
            ...
            if(valueRef != NULL) {
                ...
                UIAlertView *alert = [[UIAlertView alloc]initWithTitle:tel message:@"" delegate:self cancelButtonTitle:NSLocalizedString(@"Cancel",nil) otherButtonTitles:NSLocalizedString(@"Ortiz",nil), NSLocalizedString(@"iPhone",nil), nil];
            ...
            if(valueRef != NULL) {
                ...
                if([mailhost isEqualToString:hostname]){
                  UIAlertView *alert = [[UIAlertView alloc] initWithTitle:email message:nil delegate:self cancelButtonTitle:NSLocalizedString(@"Cancel",nil) otherButtonTitles:NSLocalizedString(@"Send Message",nil) ,NSLocalizedString(@"Send Email",nil), nil];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    switch (buttonIndex) {
        ...
        case 1:
            ...
            [[[LinphoneAppDelegate sharedAppDelegate] xmppRoster] addUser:jid withNickname:alertView.title];
            [[PhoneMainView instance] changeCurrentView:[RootViewController compositeViewDescription] push:TRUE];
